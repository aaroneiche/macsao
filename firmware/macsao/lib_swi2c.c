...
i2c_err_t swi2c_master_transmit_bytes(i2c_device_t *i2c, const uint8_t *data, uint16_t size)
{
	// Catch any bad inputs
...
i2c_err_t swi2c_master_transmit_eeprom(i2c_device_t *i2c, uint16_t address, const uint8_t *data, uint16_t size)
{
	// Catch any bad inputs
	if (i2c == NULL || data == NULL || size == 0)
		return I2C_ERR_INVALID_ARGS;
	...
	{
		// printf("first byte: %u: %02x\n", reg, *data);
		swi2c_master_tx_byte(i2c, (address >> 8)); //send high byte
		swi2c_master_tx_byte(i2c, address & 0xFF); //send low byte
		while (size)
		{
			// printf("byte %u size: %u\n", *data, size);
			swi2c_master_tx_byte(i2c, *data);
			++data;
			--size;
		}
...
i2c_err_t swi2c_master_receive_bytes(i2c_device_t *i2c, uint8_t *data, uint16_t size)
{
	// Catch any bad inputs
	if (i2c == NULL || data == NULL || size == 0)
		return I2C_ERR_INVALID_ARGS;
...
i2c_err_t swi2c_master_receive_eeprom(i2c_device_t *i2c, uint16_t address, uint8_t *data, uint16_t size)
{
	// Catch any bad inputs
	if (i2c == NULL || data == NULL || size == 0)
		return I2C_ERR_INVALID_ARGS;
	...
	if ((stat = swi2c_start(i2c)) == I2C_OK)
	{
		...
		i2c_err_t a = swi2c_master_tx_byte(i2c, i2c->address);
		printf("EEPROM Address res: %u\n",a);

		//Send teh Address to read at.
		i2c_err_t h = swi2c_master_tx_byte(i2c, (address >> 8));
		i2c_err_t l = swi2c_master_tx_byte(i2c, address & 0xFF);
		printf("high: %u, low: %u\n", h, l);

		// Repeat the START Condition
		i2c_err_t s = swi2c_start(i2c);
		printf("restart: %u\n", s);
		// Send address in Read Mode
		swi2c_master_tx_byte(i2c, i2c->address | 0x01);
